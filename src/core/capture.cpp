	...
	while (char(cv::waitKey(1)) != ' ') {
		...
		if (infoList[0]->name == "Unknown") {
			...
			if (matchedFace_on) {
				...
				if(cv::getWindowProperty("matchedFace", 0) != -1) cv::destroyWindow("matchedFace");
				//0x00007FFF5C86051C 处(位于 demo_debug.exe 中)有未经处理的异常: Microsoft C++ 异常: cv::Exception，位于内存位置 0x0000000F84D0EAC0 处。
			}
			//
			//if (50 == unknown_times) {
			//	tm.stop();
