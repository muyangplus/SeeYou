...
#include "src/cnn/metrics.h"
#include "src/cnn/architectures.h"
#include "src/cnn/train_commands.h"
...
int main(int argc, char* argv[]) {
    ...
    //const std::vector<std::string> categories({ categories_group });

    // 获取图片
    const auto images_dir = dataset_path / categories[0];
    ...
    // 保存
    const std::filesystem::path checkpoints_dir(savePath);
    ...
    for (int iter = start_iters; iter <= total_iters; ++iter) {
        ...
        // 打印信息
        ++cur_iter;
        std::printf("\rTrain===> [batch %d/%d] [loss %.3f] [Accuracy %4.3f]", iter, total_iters, mean_loss / cur_iter, train_evaluator.get());
        // 更新一次信息
        if (iter % valid_inters == 0) {
            ...
            for (int s = 1; s <= samples_num; ++s) {
                ...
                std::printf("\rValid===> [batch %d/%d] [loss %.3f] [Accuracy %4.3f]", s, samples_num, mean_valid_loss / s, valid_evaluator.get());
            }
            std::printf("\n\n");
            // 保存模型
...

    {
        ...
        for (int s = 1; s <= samples_num; ++s) {
            ...
            std::printf("\rTest===> [batch %d/%d] [loss %.3f] [Accuracy %4.3f]", s, samples_num, mean_test_loss / s, test_evaluator.get());
        }
    }
    return 0;
}
